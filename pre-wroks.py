...
def format_screenplay(text):
    ...
    for ln in lines:
        ...
        if up.startswith(('INT.','EXT.','SCENE')):
            out.append({'t':'scene','v':up})
            prev = 'scene'
            continue
        if s.isupper() and len(s.split()) <= 4:
            out.append({'t':'char','v':s})
            prev = 'char'
            continue
        if s.startswith('(') and s.endswith(')'):
            out.append({'t':'paren','v':s})
            prev = 'paren'
            continue
        if prev == 'char':
            out.append({'t':'dlg','v':s})
            prev = 'dlg'
            continue
        out.append({'t':'action','v':s})
        prev = 'action'
    return out
...
def export_pdf(project, path):
    ...
    for p in project.story.splitlines():
        elems.append(Paragraph(p.replace('', ' '), styles['Normal']))
    ...
    if project.shots:
        for i,s in enumerate(project.shots,1):
            ...
            shot_data = [
                ['Camera', s.camera, 'Lens', s.lens],
                ['Focal', s.focal_length, 'Aperture', s.aperture],
                ['Shutter', s.shutter, 'ISO', s.iso],
                ['Movement', s.movement, 'Duration', s.duration]
            ]
...
def run_smoke_tests():
    ...
    p.author = 'Tester'
    p.story = 'Line1 Line2'
    p.screenplay = 'INT. ROOM CHAR Hello'
    p.characters.append(Character('A', 30, 'Brave', 'Test Movie', 'Right'))
    p.shots.append(Shot('Opening', 'A wide shot', 'Arri', '35mm', 'f/2.8', '1/50', '100', '35mm', 'Pan', '5s'))
    tmpdir = tempfile.gettempdir()
    pj = os.path.join(tmpdir, 'test_project.fpproj')
    save_project(p, pj)
    print('Saved project to', pj)
    if reportlab is not None:
        pdfp = os.path.join(tmpdir, 'test_export.pdf')
        try:
            export_pdf(p, pdfp)
            print('Exported PDF to', pdfp)
        except Exception as e:
            print('PDF export failed during tests:', e)
    else:
...
def main():
    ...
    path = ''
    while True:
        print('Film Preproduction —', project.title)
